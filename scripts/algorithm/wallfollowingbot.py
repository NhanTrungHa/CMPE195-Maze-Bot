...
class WallFollowingBot():
    ...
    def laser_scan_callback(self, msg):
        ...
        next_x = (self.scan_zones['East-North-East'] if self.side_factor == -1 else self.scan_zones['West-North-West']) * math.sin(math.radians(23))
        next_y = (self.scan_zones['East-North-East'] if self.side_factor == -1 else self.scan_zones['West-North-West']) * math.cos(math.radians(23))
        ...
        if close_wall >= self.wall_distance * 2 and self.scan_zones['North'] < max_scan_value:
            ...
            frontal_scan = min([self.scan_zones['North'], self.scan_zones['North-North-West'] + (max_scan_value - self.scan_zones['West-North-West']), self.scan_zones['North-North-East'] + (max_scan_value - self.scan_zones['East-North-East'])])
